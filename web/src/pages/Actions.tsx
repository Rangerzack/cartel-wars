...
export default function Actions() {
  ...
  const [result, setResult] = useState<{ a: ActionDef; pay: number; rep: number; busted: boolean; heat: number; stamina: number } | null>(null)
  ...
  async function go(a: ActionDef) {
    ...
    if (r) setResult({ a, pay: r.pay, rep: r.rep, busted: r.busted, heat: r.heat, stamina: r.stamina })
  }
      ...
      {result && (
        <Modal title={result.busted && result.a.effect !== 'go_to_jail' ? 'BUSTED!' : result.a.name} onClose={() => setResult(null)}>
          {result.a.effect === 'go_to_jail' ? (
            ...
            <>
              <p className={result.rep > 0 ? 'dia' : 'gold'} style={{ fontSize: 26, fontWeight: 800, margin: '4px 0' }}>{result.rep > 0 ? `⭐ +${result.rep} reputation` : '+' + money(result.pay)}</p>
