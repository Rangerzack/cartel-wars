...
try {
  ...
  // make the first block deterministic: unclaimed, no garrison (resistance 200 vs 30 thugs ≈ 300)
  await db.query(`delete from block_garrison where block_id = (select min(id) from blocks)`)
  await db.query(`update blocks set owner_crew_id = null, taken_at = null where id = (select min(id) from blocks)`)
